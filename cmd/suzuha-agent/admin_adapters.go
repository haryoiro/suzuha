...
func (a *actionStoreAdapter) Create(ctx context.Context, aa *admin.Action) error {
	fa := &action.Action{
		ID:            aa.ID,
		ChannelID:     aa.ChannelID,
		Content:       aa.Content,
		Mode:          aa.Mode,
		ScheduledAt:   aa.ScheduledAt,
		CronExpr:      aa.CronExpr,
		RandomMinutes: aa.RandomMinutes,
...
func adminAction(v action.Action) admin.Action {
	return admin.Action{
		ID:            v.ID,
...
func (a *locationStoreAdapter) QueryLatestByUserID(ctx context.Context, userID string) ([]admin.UserLocation, error) {
	...
	for i, l := range locs {
		ul := admin.UserLocation{
			DeviceID:  l.Location.DeviceID,
