...
// summarize converts detections to a label→count map.
func summarize(detections []Detection) map[string]int {
	m := make(map[string]int)
	for _, d := range detections {
		if d.Confidence >= 0.4 { // ignore low confidence
			m[d.Label]++
		}
	}
	return m
...
// diff compares previous and current detection summaries.
// Returns labels that appeared and disappeared.
...
// buildChangeMessage creates a natural description of what changed.
