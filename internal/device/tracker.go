...
// body is excluded by default to avoid false positives (e.g. laundry).
...
type ObjectTracker struct {
	...
	currentTilt float64
	logger      *slog.Logger
	getDevice   func() *DeviceConn
}

// NewObjectTracker creates a tracker. getDevice returns the current ESP device.
func NewObjectTracker(cfg TrackerConfig, getDevice func() *DeviceConn, logger *slog.Logger) *ObjectTracker {
...
func (t *ObjectTracker) Feed(detections []Detection) {
	...
	for _, d := range detections {
		...
		if t.cfg.TargetLabel != "" {
			...
			// Cascade mode — only track labels with priority
	...
	for _, obj := range t.objects {
		...
		// Higher priority wins; within same priority, pick largest
	...
	// Only send if position actually changed (rounded to int)
	...
	if panInt == int(math.Round(t.currentPan)) && tiltInt == int(math.Round(t.currentTilt)) {
		// Sub-degree movement, accumulate but don't send
		t.currentPan = newPan
	...
	// Send servo command async to avoid blocking the detection pipeline.
	go func() {
		dev := t.getDevice()
		if dev == nil {
			return
		}
		_ = dev.SendCommand(map[string]any{
			"cmd":  "servo",
			"pan":  panInt,
			"tilt": tiltInt,
		})
...
func (t *ObjectTracker) updateMatched(obj *trackedObject, d Detection) {
	...
	// EMA smoothing — use label-aware target point
	tx, ty := targetPoint(d)
