...
func (h *AdminHandler) BoredomGet(ctx context.Context) (*gen.BoredomStatus, error) {
	const (
		boredomRate = 8.0
		boredomMax  = 100.0
	)

	resp := &gen.BoredomStatus{
		PostThreshold: 20.0,
		Boredom:       boredomMax,
	}
	...
	if err == nil {
		resp.LastInteraction = gen.NewOptString(lastMsg.Format(time.RFC3339))
	...
	if err == nil {
		...
		if json.Unmarshal([]byte(stateJSON), &state) == nil && !state.LastPostedAt.IsZero() {
			resp.LastPostedAt = gen.NewOptString(state.LastPostedAt.Format(time.RFC3339))
		}
	}
