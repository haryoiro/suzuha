...
func (h *AdminHandler) ChannelSettingsList(ctx context.Context, params gen.ChannelSettingsListParams) (*gen.ChannelSettingsListOK, error) {
	...
	var entries []gen.ChannelSetting
	for rows.Next() {
		var e gen.ChannelSetting
		var mode string
		...
		e.Mode = gen.ChannelSettingMode(mode)
		if lastMsg.Valid {
			e.LastUserMessageAt = gen.NewOptString(lastMsg.Time.Format("2006-01-02 15:04:05"))
		}
		if settingsUpdated.Valid {
			e.SettingsUpdatedAt = gen.NewOptString(settingsUpdated.Time.Format("2006-01-02 15:04:05"))
		}
		entries = append(entries, e)
	}
	if entries == nil {
		entries = []gen.ChannelSetting{}
	}
	return &gen.ChannelSettingsListOK{Data: entries}, nil
}

func (h *AdminHandler) ChannelSettingsUpdate(ctx context.Context, req *gen.UpdateChannelSettingRequest, params gen.ChannelSettingsUpdateParams) (*gen.OkResponse, error) {
	mode := string(req.Mode.Or(gen.UpdateChannelSettingRequestModeActive))
...
func (h *AdminHandler) ChannelSettingsDelete(ctx context.Context, params gen.ChannelSettingsDeleteParams) error {
	_, err := h.db.ExecContext(ctx,
